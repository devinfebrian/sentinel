...
export interface AnalysisEvent {
   ...
   * between started and done, only those two instants. The query/detection
   * step that runs ahead of these for every transaction has no event of its
   * own — it is a sub-millisecond synchronous DB call, so a started/done
   * pair around it would always land back-to-back with no visible gap. The
   * Query pipeline node is driven by the run's overall status instead — see
   * FindingsClient.
   */
  agent?: AgentName;
